...
export async function POST(request: NextRequest) {
  ...
  if (!authError && authResult.user) {
    ...
    const { userId: existingAuthUserId, error: authLookupError } = await findAuthUserIdByEmail(
      adminClient,
      normalizedEmail
    )
    ...
    if (authLookupError) {
      return NextResponse.json(
        {
          error: authLookupError.message || "Failed to resolve existing auth user",
        },
        { status: 500 }
      )
    }
    ...
    const { error: updateExistingError } = await adminClient.auth.admin.updateUserById(resolvedUserId, {
      password: parsed.data.password,
      email_confirm: true,
      user_metadata: {
        full_name: name,
    ...
    if (updateExistingError) {
      return NextResponse.json(
        { error: updateExistingError.message || "Failed to update existing auth user" },
        { status: 500 }
      )
    }
